...
// A 'command' schedule runs arbitrary console input, so creating/editing one
// is also gated on console:write — otherwise schedules:manage alone would
// let an admin without console access get the same effect indirectly.
function canUseCommandAction(req) {
  return req.user.role === 'super_admin' || hasPermission(req.user.sub, 'console:write');
}
...
router.post('/:id', verifyToken, requirePermission('schedules:manage'), async (req, res) => {
  const game = getGame(req.params.id);
  if (!game) return res.status(404).json({ error: 'Game not found' });
  ...
  if (!['start', 'stop', 'restart', 'command', 'backup'].includes(action)) {
    return res
      .status(400)
      .json({ error: 'action must be start, stop, restart, command, or backup' });
  }
  if (action === 'command' && !command?.trim()) {
    return res.status(400).json({ error: 'command is required for command action' });
  }
  if (action === 'command' && !canUseCommandAction(req)) {
...
router.put(
  ...
  async (req, res) => {
    ...
    if (
      action !== undefined &&
      !['start', 'stop', 'restart', 'command', 'backup'].includes(action)
    ) {
      return res
        .status(400)
        .json({ error: 'action must be start, stop, restart, command, or backup' });
...
router.post(
  ...
  async (req, res) => {
    ...
    const existing = (game.schedules ?? []).find((s) => s.id === req.params.scheduleId);
    if (!existing) return res.status(404).json({ error: 'Schedule not found' });
    if (existing.action === 'command' && !canUseCommandAction(req)) {
      return res.status(403).json({ error: 'Insufficient permissions' });
    }
    ...
    try {
      ...
      res
        .status(502)
        .json({ error: err.message, schedule: schedule ? withNextRun(schedule) : undefined });
    }
  }
);
