...
type TabItem = string | { label: string; value: string; count?: number };
...
export function Tabs({ tabs = [], value, onChange, className = '', ...rest }: TabsProps) {
  const items = tabs.map((t) => (typeof t === 'string' ? { label: t, value: t, count: undefined } : t));
  return (
      ...
      {items.map((t) => {
        ...
        return (
          <button
            ...
            className={`bg-transparent border-b-2 px-4 py-3 text-sm font-semibold tracking-[.02em] cursor-pointer transition-colors ${
